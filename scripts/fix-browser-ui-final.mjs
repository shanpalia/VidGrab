...
// Keep YouTube's search/header UI intact. Only remove its bottom pivot navigation.
s = s.replace(
...
// Android navigation bar: use immersive sticky on every supported API level and
// lay the app out behind the navigation area so no system navigation strip is shown.
...
if (s.includes(oldHide)) s = s.replace(oldHide, newHide);

// Re-hide after keyboard/focus/window transitions.
s = s.replace(
...
console.log('[fix-browser-ui-final] search/header preserved; YouTube bottom pivot hidden; Android navigation bar forced immersive');

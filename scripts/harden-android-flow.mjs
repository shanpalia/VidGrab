...
// Browser is a full-screen mode; keep the main VidGrab bottom navigation only.
...
// Remove the browser's second bottom navigation bar. The main VidGrab BottomNav remains visible.
if (!browser.includes('VIDGRAB_BROWSER_BOTTOM_NAV_REMOVED_V1')) {
  ...
  if (navStart >= 0 && rootEnd >= 0) {
    browser = browser.slice(0, navStart) + '      {/* VIDGRAB_BROWSER_BOTTOM_NAV_REMOVED_V1 */}\n' + browser.slice(rootEnd);
  }
}
...
// Use several currently listed public Invidious instances in parallel before the slower fallbacks.
if (!api.includes('VIDGRAB_INVIDIOUS_FAST_V2')) {
  ...
  const fastBlock = [
    ...
    "    const invidiousInstances = ['https://inv.nadeko.net','https://invidious.nerdvpn.de','https://yt.chocolatemoo53.com','https://invidious.tiekoetter.com'];",
    '    const invidiousRequests = invidiousInstances.map(async (base) => {',
...
// Keep the existing Invidious fallback block for builds where the fast parallel block is unavailable.

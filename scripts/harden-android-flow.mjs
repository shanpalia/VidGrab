...
// Public Invidious instances are an additional real-search fallback. The official
// Invidious API exposes /api/v1/search and returns videoId/title/author/thumbnails.
if (!api.includes('VIDGRAB_INVIDIOUS_FALLBACK_V1')) {
  ...
  const replacement = `    /* VIDGRAB_INVIDIOUS_FALLBACK_V1 */
    ...
    for (const base of invidiousInstances) {
      try {
        const res = await fetch(`${'${base}'}/api/v1/search?q=${'${encodeURIComponent(q)}'}&type=video&region=IN&hl=en-US`, { signal: AbortSignal.timeout(7000) });
        if (!res.ok) continue;

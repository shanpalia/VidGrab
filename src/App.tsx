...
export default function App() {
  ...
  useEffect(() => {
    const onNativeGrab = (event: Event) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-gray-900 flex flex-col font-sans overflow-x-hidden">
      ...
      {errorMessage && <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-[150] bg-red-600 text-white px-4 py-2 rounded-full text-xs font-bold shadow-xl max-w-[90vw] text-center">{errorMessage}</div>}
    </div>
  );
}
